...
{
  var _data = [];
  var _objs = RecurseObjects(data, parentCode)

  _objs.forEach(itm=>{
    var _subtype = itm.subtype == undefined ? "[REDACTED]" : itm.subtype.type;
    var _subname = itm.subtype == undefined ? "[REDACTED]" : itm.subtype.name;
    _data.push({
      "id":itm.id,
      ...
      "type":itm.type == null ? "[REDACTED]" : itm.code,
      "subtype":_subtype,
          ...
          if (i.toLowerCase() == "code" && obj[i].includes("."))
          {
            if(obj[i].split(".")[0].toLowerCase() == parentCode.toLowerCase()){
              if (objects.lastIndexOf(obj) == -1){
                obj.parent_code=parentCode.toUpperCase();
                objects.push(obj);
              }
