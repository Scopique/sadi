...
const files = {
  ...
  shipsMaster: "./scdata/shipsMaster.json"
}
...
function GenShipsMaster(){
  var _data = [];
  var _shipData = readFile(files.ships);
  ...
  _shipData.data.forEach(itm=>{    
    ...
    if (itm!=null){
      ...
      _data.push({
        ...
        "cargocapacity":itm.cargocapacity,
        "description":itm.description,
        "focus":itm.focus,
        "manufacturer":_mfrName,
        "logo":_mfrLogo,
        "crew":itm.min_crew + "/" + itm.max_crew,
        "banner":itm.media[0].images.banner,
        "price":itm.price,
        "production_status":itm.production_status,
        "size":itm.size,
        "url":itm.url,
        "type":itm.type
      })
  ...
  fs.writeFileSync(files.shipsMaster, JSON.stringify(_data));
}
