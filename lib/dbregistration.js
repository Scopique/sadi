/***************************************************************
 *                       REGISTRATION
 * Handles the database access needed to collect and store, 
 * update, and remove guild-related data to/from our centeral
 * data store.
 **************************************************************/
...
module.exports = {
  ...
  GuildUnregister:function(gid){
    //Delete the guild, users, and roles (and user roles)
    const _db = DBQuery();

    try {
      _db.query(mysql.format('DELETE FROM sadi_guild_user_roles WHERE guildID=?', [gid]))
      .then(result=>{
        _db.query(mysql.format('DELETE FROM sadi_guild_users WHERE guildID=?',[gid]))
        .then(result=>{
          _db.query(msqlformat('DELETE FROM sadi_guild_roles WHERE guildID=?', [gid]))
          .then(result=>{
            _db.query('DELETE FROM sadi_guild WHERE guildID=?',[gid])
            .then(result=>{
              //Done. Everything should have been deleted.
            })
            .catch(err=>{
              console.log(`Error removing guild=>${err}`);
            })
          })
          .catch(err=>{
      ...
      .catch(err=>{
        console.log(`Error removing user roles=>${err}`);
      });
    }catch(err){
      console.log(`Guild unregister error=>${err}`);
    }
  },
  GuildUpdate:function(guild){
    const _timestamp = {toSqlString:function() { return 'CURRENT_TIMESTAMP()';}}
    const _db = DBQuery();

    try{
      return(
        _db.query('UPDATE sadi_guild SET name=?, ownerID=?, lastDBUpdate=? WHERE id=?', [guild.name, guild.ownerID, _timestamp, guild.id])
        .then(result=>{
          //Guild updated successfully.
          return ReturnDataObject(
            "OK",`Guild '${guild.name}' has been updated.`,`Guild '${guild.name}' has been updated.`,{}
          )
        })
        .catch(err=>{
          console.log(`Error updating guild '${guild.name}'=>${err}`);
        })
      )
    }catch(err){
      console.log(`Guild '${guild.name}' update error=> =>${err}`);
    }
  },
  MemberRegister:function(gid, member){
    const _timestamp = {toSqlString:function() { return 'CURRENT_TIMESTAMP()';}}
    const _db = DBQuery();

    try{
      return(
        _db.query(mysql.format("INSERT INTO sadi_guild_users SET id=?, guildID=?, username=?, discriminator=?, nickname=?, avatarURL=?, displayAvatarURL=?, lastDBUpdate=?", [member.user.id, gid, member.user.username, member.user.discriminator, member.user.nickname, member.user.avatarURL, member.user.displayAvatarURL, _timestamp]))
        .then(result=>{
          //Need to register them with whatever role they get when they first join the server.
          member._roles.forEach(role=>{this.AssignRole(gid, member.id, role.id);})

          return ReturnDataObject(
            "OK",`Member ${member.user.username} has been registered.`, `Member ${member.user.username} has been registered to guild ID ${gid}`, {}
          )
        })
        .catch(err=>{
  ...
  MemberUnregister:function(gid, mid){
    const _db = DBQuery();

    try {
      return(
        _db.query(mysql.format('DELETE FROM sadi_guild_user_roles WHERE userID=? AND guildID=?',[mid, gid]))
        .then(result=>{
          return (
            _db.query(mysql.format('DELETE FROM sadi_guild_users WHERE id=? and guildID=?',[mid,gid]))
            .then(result=>{
              return ReturnDataObject( "OK",`Member has been removed.`,`Member ID ${mid} has been removed from Guild ID ${gid}`,{} )
            })
            .catch(err=>{
      ...
      console.log(`member Unregister error=>${err}`)
    }
  },
  MemberUpdate:function(gid,member){
    const _timestamp = {toSqlString:function() { return 'CURRENT_TIMESTAMP()';}}
    const _db = DBQuery();

    try{
      return(
        _db.query(mysql.format("UPDATE sadi_guild_users SET nickname=?, avatarURL=?, displayAvatarURL=?, lastDBUpdate=? WHERE id=? AND guildID=?", [member.user.nickname, member.user.avatarURL, member.user.displayAvatarURL, _timestamp, member.user.id, gid]))
        .then(result=>{
          return ReturnDataObject(
            "OK",`Member ${member.username} has been updated.`, `Member ${member.username} has been updated`, {}
          )
        })
        .catch(err=>{
  ...
  RoleRegister:function(gid, role){
    const _timestamp = {toSqlString:function() { return 'CURRENT_TIMESTAMP()';}}
    const _db = DBQuery();

    try{
      return(
        _db.query(mysql.format('INSERT INTO sadi_guild_roles SET id=?, guildID=?, name=?, lastDBUpdate=?',[role.id, gid, role.name, _timestamp]))
        .then(result=>{
          return ReturnDataObject("OK",`Role ${role.name} has been registered.``Role ${role.name} has been registered to Guild ID ${gid}`, {});
        })
        .catch(err=>{
      ...
      console.log(`Role Register error =>${err}`);
    }
  },
  RoleUnregister:function(gid, rid){
    const _timestamp = {toSqlString:function() { return 'CURRENT_TIMESTAMP()';}}
    const _db = DBQuery();

    try{
      return(
        _db.query(mysql.format('DELETE FROM sadi_guild_user_roles WHERE roleID=? AND guildID=?',[rid, gid]))
        .then(result=>{
          _db.query(mysql.format('DELETE FROM sadi_guild_roles WHERE id=? and guildID=?',[rid,gid]))
          .then(result=>{
            return ReturnDataObject( "OK",`Role has been removed.`,`Role has been removed`,{} )
          })
          .catch(err=>{
      ...
      console.log(`Role Unregister error =>${err}`)
    }
  },
  RoleUpdate:function(gid,role){
    const _timestamp = {toSqlString:function() { return 'CURRENT_TIMESTAMP()';}}
    const _db = DBQuery();

    try{
      return(
        _db.query(mysql.format("UPDATE sadi_guild_roles SET name=?, lastDBUpdate=? WHERE id=? AND guildID=?", [role.name, _timestamp, role.id, gid]))
        .then(result=>{
          return ReturnDataObject(
            "OK",`Role ${role.name} has been updated.`, `Role ${role.name} has been updated`, {}
          )
        })
        .catch(err=>{
  ...
  AssignRole:function(gid, mid, rid){
    const _timestamp = {toSqlString:function() { return 'CURRENT_TIMESTAMP()';}}
    const _db = DBQuery();

    try{
      return(
        _db.query(mysql.format("INSERT INTO sadi_guild_user_roles SET guildID=?, userID=?, roleID=?, lastDBUpdate=?", [gid, mid, rid, _timestamp]))
        .then(result=>{
          return ReturnDataObject(
            "OK",`Role has been assigned to user.`, `Role ID ${rid} has been assigned to User ID ${mid} in Guild ID ${gid}`, {}
          )
        })
        .catch(err=>{
          if (err.code == 'ER_DUP_ENTRY'){
            console.log(`Role ${rid} exists for User ID ${mid} in Guild ID ${gid}. Record was not inserted.`);
          }
        })
      )
    }catch(err){
  ...
  RemoveRole:function(gid, mid, rid){
    const _timestamp = {toSqlString:function() { return 'CURRENT_TIMESTAMP()';}}
    const _db = DBQuery();

    try{
      return(
        _db.query(mysql.format('DELETE FROM sadi_guild_user_roles WHERE roleID=? AND userID=? AND guildID=?',[rid, mid, gid]))
        .then(result=>{
          return ReturnDataObject( "OK",`Role has been removed from user.`,`Role ID ${rid} has been assigned to User ID ${mid} in Guild ID ${gid}`,{} )
        })
        .catch(err=>{
          console.log(`Error removing role from user =>${err}`)
        })
      )
    }catch(err){
